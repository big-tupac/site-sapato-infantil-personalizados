...
const filterContextBlockAttributes = (schema, suggestions) => {
    ...
    const isShardKeyAvailable = Boolean(datasourceProvider && datasourceProvider.includes('mysql') && previewFeatures?.includes('shardkeys'));
 ...
 */
function filterSuggestionsForLine(suggestions, currentLine, fieldType, fieldBlockType, schema) {
    const datasourceProvider = (0, ast_1.getFirstDatasourceProvider)(schema);
    const previewFeatures = (0, ast_1.getAllPreviewFeaturesFromGenerators)(schema);
    const isShardKeyAvailable = Boolean(datasourceProvider && datasourceProvider.includes('mysql') && previewFeatures?.includes('shardkeys'));
    if (!isShardKeyAvailable) {
